# Generated by Django 3.2.25 on 2024-10-04 10:57

from django.db import migrations


class Migration(migrations.Migration):

    dependencies = [
        ('assistant', '0045_update_renewal_emails'),
    ]

    operations = [
        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_reviewers_startup_txt',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'PLAIN',
    template  = e'<p>{% if role == roles.RESEARCH %}Madame la pr&eacute;sidente, Monsieur le pr&eacute;sident de l&#39;institut {{ entity }},{% elif role == roles.RESEARCH_ASSISTANT %}Madame la d&eacute;l&eacute;gu&eacute;e de pr&eacute;sident, Monsieur le d&eacute;l&eacute;gu&eacute; de pr&eacute;sident d&#39;institut,{% elif role == roles.SUPERVISION %}Madame la doyenne, Monsieur le doyen de la facult&eacute; {{ entity }},{% elif roles.SUPERVISION_ASSISTANT in role %}Madame la d&eacute;l&eacute;gu&eacute;e de doyen, Monsieur le d&eacute;l&eacute;gu&eacute; de doyen de facult&eacute;,{% elif role == roles.VICE_RECTOR %}Madame la vice-rectrice, Monsieur le vice-recteur,{% else %}Madame, Monsieur,{% endif %}</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
Nous venons de lancer la proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }}.<br />
Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante :&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a><br />
<br />
Pour rappel, toute demande de renouvellement introduite par un&middot;e assistant&middot;e doit &ecirc;tre mise dans le circuit et ne peut &ecirc;tre bloqu&eacute;e sous pr&eacute;texte que les avis sont n&eacute;gatifs car l&#39;assistant&middot;e a, dans ce cas, le droit d&#39;introduire un recours selon l&#39;article 50 du RAMCS.</p>

<p><br />
Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Pour le site de Saint Louis - Contact : Catherine Demain</p>'
WHERE id = 68;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_dean_assistant_decline_html',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>Dear Dean,</p>
&nbsp;

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.<br />
<br />
The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.<br />
<br />
You receive this message because {{ assistant }} has declined his renewal application.</p>

<p>Regards,<br />
The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
If you are a dean on the Saint-Louis Site, your contact is Catherine Demain<br />
{% endautoescape %}</p>'
WHERE id = 77;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_dean_assistant_decline_txt',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'PLAIN',
    template  = e'<p>Madame la doyenne, Monsieur le doyen,</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }} vient de d&eacute;buter. Elle s&#39;effectue int&eacute;gralement par voie &eacute;lectronique.<br />
<br />
Vous recevez ce message car&nbsp;{{ assistant }} a d&eacute;clin&eacute; sa demande de renouvellement.</p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Si vous &ecirc;tes doyen&middot;ne sur le site de Saint-Louis, votre contact est Catherine Demain</p>'
WHERE id = 76;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_normal_renewal_txt',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'PLAIN',
    template  = e'<p>&nbsp;</p>

<p>Bonjour {{ first_name}} {{ last_name}},</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }} vient de d&eacute;buter. Celle-ci s&rsquo;effectue int&eacute;gralement par voie &eacute;lectronique.<br />
<br />
Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page&nbsp;suivante :&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Si vous avez &eacute;t&eacute; engag&eacute; sur le site de Saint-Louis, votre contact est Catherine Demain</p>'
WHERE id = 60;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_normal_renewal_txt',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'PLAIN',
    template  = e'<p>Hello {{ first_name}} {{ last_name}},</p>

<p>&nbsp;</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.<br />
<br />
The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }}&nbsp;and the {{ end_date }} has just begun. It is conducted by electronic means only.<br />
<br />
In order to gain access to the procedure, simply click on the following link:&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Regards,<br />
The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
If you have been hired on the Saint-Louis site, the contact is Catherine Demain</p>'
    WHERE id = 62;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_normal_renewal_html',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>Hello {{ first_name}} {{ last_name}},</p>

<p>&nbsp;</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.<br />
<br />
The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }}&nbsp;and the {{ end_date }} has just begun. It is conducted by electronic means only.<br />
<br />
In order to gain access to the procedure, simply click on the following link:&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Regards,<br />
The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
If you have been hired on the Saint-Louis site, the contact is Catherine Demain<br />
{% endautoescape %}</p>'
WHERE id = 61;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_reviewers_startup_html',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>{% if role == roles.RESEARCH %}Madame la pr&eacute;sidente, Monsieur le pr&eacute;sident de l&#39;institut {{ entity }},{% elif role == roles.RESEARCH_ASSISTANT %}Madame la d&eacute;l&eacute;gu&eacute;e de pr&eacute;sident, Monsieur le d&eacute;l&eacute;gu&eacute; de pr&eacute;sident d&#39;institut,{% elif role == roles.SUPERVISION %}Madame la doyenne, Monsieur le doyen de la facult&eacute; {{ entity }},{% elif roles.SUPERVISION_ASSISTANT in role %}Madame la d&eacute;l&eacute;gu&eacute;e de doyen, Monsieur le d&eacute;l&eacute;gu&eacute; de doyen de facult&eacute;,{% elif role == roles.VICE_RECTOR %}Madame la vice-rectrice, Monsieur le vice-recteur,{% else %}Madame, Monsieur,{% endif %}</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
Nous venons de lancer la proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }}.<br />
<br />
Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante :&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a><br />
<br />
Pour rappel, toute demande de renouvellement introduite par un&middot;e assistant&middot;e doit &ecirc;tre mise dans le circuit et ne peut &ecirc;tre bloqu&eacute;e sous pr&eacute;texte que les avis sont n&eacute;gatifs car l&#39;assistant&middot;e a, dans ce cas, le droit d&#39;introduire un recours selon l&#39;article 50 du RAMCS.</p>

<p><br />
Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Pour le site de Saint Louis - Contact : Catherine Demain<br />
{% endautoescape %}</p>'
WHERE id = 67;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_normal_renewal_html',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>Bonjour {{ first_name}} {{ last_name}},</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }} vient de d&eacute;buter. Celle-ci s&rsquo;effectue int&eacute;gralement par voie &eacute;lectronique.<br />
<br />
Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page&nbsp;suivante :&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Si vous avez &eacute;t&eacute; engag&eacute; sur le site de Saint-Louis, votre contact est Catherine Demain {% endautoescape %}</p>'
WHERE id = 59;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_phd_supervisor_txt',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'PLAIN',
    template  = e'<p>Madame la professeure, Monsieur le professeur,</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }} vient de d&eacute;buter. Elle s&#39;effectue int&eacute;gralement par voie &eacute;lectronique.<br />
<br />
Vous recevez ce message en tant que promoteur&middot;rice de recherche pour {{ assistant }}.<br />
<br />
Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante : <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Si vous &ecirc;tes promoteur&middot;rice de recherche d&#39;un&middot;e assistant&middot;e engag&eacute;&middot;e sur le site de Saint-Louis - Contact : Catherine Demain</p>'
WHERE id = 72;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_phd_supervisor_html',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>Dear Professor,</p>

<p>&nbsp;</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.<br />
<br />
The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.<br />
<br />
You receive this message as a supervisor for {{ assistant }}.<br />
<br />
In order to gain access to the procedure, simply click on the following link:&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a></p>

<p><br />
Regards,</p>

<p>The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
If you are supervisor for an assistant hired on the Saint-Louis site - Contact Catherine Demain {% endautoescape %}</p>'
WHERE id = 73;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_phd_supervisor_html',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>Madame la professeure, Monsieur le professeur,</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }} vient de d&eacute;buter. Elle s&#39;effectue int&eacute;gralement par voie &eacute;lectronique.<br />
<br />
Vous recevez ce message en tant que promoteur&middot;rice de recherche pour {{ assistant }}.<br />
<br />
Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante :&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Si vous &ecirc;tes promoteur&middot;rice de recherche d&#39;un&middot;e assistant&middot;e engag&eacute;&middot;e sur le site de Saint-Louis - Contact : Catherine Demain<br />
{% endautoescape %}</p>'
WHERE id = 71;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_dean_assistant_decline_html',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>Madame la doyenne, Monsieur le doyen,</p>

<p>&nbsp;</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.<br />
<br />
La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }}&nbsp;et le {{ end_date }} vient de d&eacute;buter. Elle s&#39;effectue int&eacute;gralement par voie &eacute;lectronique.<br />
<br />
Vous recevez ce message car&nbsp;{{ assistant }} a d&eacute;clin&eacute; sa demande de renouvellement.</p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact : Marie-Dominique Heureux<br />
Si vous &ecirc;tes doyen&middot;ne sur le site de Saint-Louis, votre contact est Catherine Demain {% endautoescape %}</p>'
WHERE id = 75;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_dean_assistant_decline_txt',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'PLAIN',
    template  = e'<p>Dear Dean,</p>
&nbsp;

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.<br />
<br />
The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.<br />
<br />
You receive this message because {{ assistant }} has declined his renewal application.</p>
<br />
Regards,<br />
The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
If you are a dean on the Saint-Louis Site, your contact is Catherine Demain'
WHERE id = 78;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_phd_supervisor_txt',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'PLAIN',
    template  = e'<p>Dear Professor,</p>

<p>&nbsp;</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.<br />
<br />
The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.<br />
<br />
You receive this message as a supervisor for {{ assistant }}.<br />
<br />
In order to gain access to the procedure, simply click on the following link:&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a></p>

<p><br />
Regards,</p>

<p>The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
If you are supervisor for an assistant hired one the Saint-Louis site - Contact Catherine Demain</p>

<p>&nbsp;</p>'
WHERE id = 74;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_reviewers_startup_txt',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'PLAIN',
    template  = e'<p>{% if role == roles.RESEARCH %}Dear President of {{ entity }},{% elif role == roles.RESEARCH_ASSISTANT %}Dear Representative of the Institute President,{% elif role == roles.SUPERVISION %}Dear Dean of {{ entity }},{% elif role == roles.SUPERVISION_ASSISTANT %}Dear Dean of Faculty representative,{% elif role == roles.VICE_RECTOR %}Dear Vice Rector,{% else %}Dear Madam, Dear Sir,{% endif %}</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.</p>

<p>We have just started the procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }}.</p>

<p>In order to gain access to the procedure, simply click on the following link:&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a></p>

<p>As a reminder, any renewal application made by an assistant must be put in the circuit and cannot be blocked on the pretext of negative opinions since the assistant has, in this case, the right of appeal against the decision under article 50 of the RAMCS.</p>

<p><br />
Regards,</p>

<p>The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
For the Saint-Louis site - Contact Catherine Demain</p>'
WHERE id = 70;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_reviewers_startup_html',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'HTML',
    template  = e'<p>{% autoescape off %}</p>

<p>{% if role == roles.RESEARCH %}Dear President of {{ entity }},{% elif role == roles.RESEARCH_ASSISTANT %}Dear Representative of the Institute President,{% elif role == roles.SUPERVISION %}Dear Dean of {{ entity }},{% elif role == roles.SUPERVISION_ASSISTANT %}Dear Dean of Faculty representative,{% elif role == roles.VICE_RECTOR %}Dear Vice Rector,{% else %}Dear Madam, Dear Sir,{% endif %}</p>

<p>&nbsp;</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.</p>

<p>We have just started the procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }}.</p>

<p>In order to gain access to the procedure, simply click on the following link:&nbsp;<a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212">https://uclouvain.atlassian.net/servicedesk/customer/portal/12/article/947421212</a></p>

<p>As a reminder, any renewal application made by an assistant must be put in the circuit and cannot be blocked on the pretext of negative opinions since the assistant has, in this case, the right of appeal against the decision under article 50 of the RAMCS.</p>

<p>Regards,<br />
The Human Resources<br />
Contact : Marie-Dominique Heureux<br />
For the Saint-Louis site - Contact Catherine Demain</p>
{% endautoescape %}'
WHERE id = 69;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_special_renewal_html',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'HTML',
    template  = e'{% autoescape off %}
<p>Hello {{ first_name }} {{ last_name }},</p>

<p>The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.</p>

<p>In your case, you are in a <b>special renewal procedure</b> : you have almost reached the maximum number of years authorized for a mandate but have yet to surpass it. If you so desire, you can apply for a a renewal period of one year, and this, without having to give a particular justification.</p>

<p>In order to gain access to the procedure, simply click on the following link: <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Regards,<br />
The Human Resources<br />
Contact: Marie-Dominique Heureux<br />
If you have been hired on the Saint-Louis site, the contact is Catherine Demain<br />
{% endautoescape %}</p>'
WHERE id = 65;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_special_renewal_txt',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'PLAIN',
    template  = e'<p>Hello {{ first_name }} {{ last_name }},</p>

<p>The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.</p>

<p>In your case, you have reached the maximum number of years allowed to apply for a normal renewal. You may however, if you so desire and if it can be justified by exceptional circumstances, apply for a one-off renewal period of one year. Therefore, we ask you to provide detailed arguments in order to facilitate the decision of the authorities on the exceptional nature or not of these circumstances.</p>

<p>In order to gain access to the procedure, simply click on the following link: <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Regards,<br />
The Human Resources<br />
Contact: Marie-Dominique Heureux<br />
If you have been hired on the Saint-Louis site, the contact is Catherine Demain</p>'
WHERE id = 66;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_special_renewal_html',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'HTML',
    template  = e'{% autoescape off %}
<p>Bonjour {{ first_name }} {{ last_name }},</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.</p>

<p>La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }} et le {{ end_date }} vient de d&eacute;buter. Celle-ci s&rsquo;effectue int&eacute;gralement par voie &eacute;lectronique.</p>

<p>En ce qui vous concerne, vous &ecirc;tes dans le cas d&rsquo;une demande de <b>renouvellement sp&eacute;cial</b> : vous &ecirc;tes proche du nombre maximum d&rsquo;ann&eacute;es de mandat autoris&eacute;es sans pour autant avoir d&eacute;pass&eacute; ce nombre. Si vous le souhaitez, vous pouvez donc introduire une demande de renouvellement sp&eacute;cial pour une dur&eacute;e de 1 an, et ce sans donner de justification particuli&egrave;re.</p>

<p>Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante : <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact: Marie-Dominique Heureux<br />
Si vous avez &eacute;t&eacute; engag&eacute; sur le site de Saint-Louis, votre contact est Catherine Demain<br />
{% endautoescape %}</p>'
WHERE id = 63;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_special_renewal_txt',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'PLAIN',
    template  = e'<p>Bonjour {{ first_name }} {{ last_name }},</p>
<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.</p>
<p>La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }} et le {{ end_date }} vient de d&eacute;buter. Celle-ci s&rsquo;effectue int&eacute;gralement par voie &eacute;lectronique.</p>
<p>En ce qui vous concerne, vous &ecirc;tes dans le cas d&rsquo;une demande de renouvellement sp&eacute;cial : vous &ecirc;tes proche du nombre maximum d&#39;ann&eacute;es de mandat autoris&eacute;es sans pour autant avoir d&eacute;pass&eacute; ce nombre. Si vous le souhaitez, vous pouvez donc introduire une demande de renouvellement sp&eacute;cial pour une dur&eacute;e de 1 an, et ce sans donner de justification particuli&egrave;re.</p>
<p>Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante : <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact: Marie-Dominique Heureux<br />
Si vous avez &eacute;t&eacute; engag&eacute; sur le site de Saint-Louis, votre contact est Catherine Demain</p>'
WHERE id = 64;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_except_renewal_html',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'HTML',
    template  = e'{% autoescape off %}
<p>Bonjour {{ first_name }} {{ last_name }},</p>

<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.</p>

<p>La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }} et le {{ end_date }} vient de d&eacute;buter. Celle-ci s&#39;effectue int&eacute;gralement par voie &eacute;lectronique.</p>

<p>Dans votre cas, vous avez atteint le nombre maximum d&#39;ann&eacute;es de mandat autoris&eacute;es pour demander un renouvellement normal, donc si vous le souhaitez et si vous avez des circonstances exceptionnelles qui le justifient, vous pouvez introduire une demande de renouvellement exceptionnel d&#39;un an. Pour cela, nous vous demandons d&#39;apporter une argumentation approfondie pour faciliter la d&eacute;cision des autorit&eacute;s sur le caract&egrave;re exceptionnel ou non de ces circonstances.</p>

<p>Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante : <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact: Marie-Dominique Heureux<br />
Si vous avez &eacute;t&eacute; engag&eacute; sur le site de Saint-Louis, votre contact est Catherine Demain</p>
{% endautoescape %}'
WHERE id = 170;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_except_renewal_txt',
    subject   = 'Renouvellement des mandats des assistant·e·s',
    language  = 'fr-be',
    format    = 'PLAIN',
    template  = e'<p>Bonjour {{ first_name }} {{ last_name }},</p>
<p>Ceci est un message automatique g&eacute;n&eacute;r&eacute; par le serveur OSIS &ndash; Merci de ne pas y r&eacute;pondre.</p>
<p>La proc&eacute;dure de renouvellement des mandats des assistant&middot;e&middot;s dont le contrat arrive &agrave; &eacute;ch&eacute;ance entre le {{ start_date }} et le {{ end_date }} vient de d&eacute;buter. Celle-ci s&rsquo;effectue int&eacute;gralement par voie &eacute;lectronique.</p>
<p>Dans votre cas, vous avez atteint le nombre maximum d&#39;ann&eacute;es de mandat autoris&eacute;es pour demander un renouvellement normal, donc si vous le souhaitez et si vous avez des circonstances exceptionnelles qui le justifient, vous pouvez introduire une demande de renouvellement exceptionnel d&#39;un an. Pour cela, nous vous demandons d&#39;apporter une argumentation approfondie pour faciliter la d&eacute;cision des autorit&eacute;s sur le caract&egrave;re exceptionnel ou non de ces circonstances .</p>
<p>Pour acc&eacute;der &agrave; la proc&eacute;dure, il vous suffit de vous rendre sur le portail &agrave; la page suivante : <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Cordialement,<br />
Les Ressources Humaines<br />
Contact: Marie-Dominique Heureux<br />
Si vous avez &eacute;t&eacute; engag&eacute; sur le site de Saint-Louis, votre contact est Catherine Demain</p>'
WHERE id = 171;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_except_renewal_html',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'HTML',
    template  = e'{% autoescape off %}
<p>Hello {{ first_name }} {{ last_name }},</p>
<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.</p>
<p>The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.</p>
<p>In your case, you have reached the maximum number of years allowed to apply for a normal renewal. You may however, if you so desire and if it can be justified by exceptional circumstances, apply for a one-off renewal period of one year. Therefore, we ask you to provide detailed arguments in order to facilitate the decision of the authorities on the exceptional nature or not of these circumstances.</p>
<p>In order to gain access to the procedure, simply click on the following link: <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Regards,<br />
The Human Resources<br />
Contact: Marie-Dominique Heureux<br />
If you have been hired on the Saint-Louis site, the contact is Catherine Demain</p>
{% endautoescape %}'
WHERE id = 172;"""),

        migrations.RunSQL("""
UPDATE public.osis_common_messagetemplate
SET reference = 'assistant_assistants_startup_except_renewal_txt',
    subject   = 'Assistants mandates renewal',
    language  = 'en',
    format    = 'PLAIN',
    template  = e'<p>Hello {{ first_name }} {{ last_name }},</p>

<p>This is an automatic message generated by the OSIS server &ndash; Please do not reply to this message.</p>
<p>The procedure for renewing the mandates of assistants whose contract expires between the {{ start_date }} and the {{ end_date }} has just begun. It is conducted by electronic means only.</p>
<p>In your case, you have reached the maximum number of years allowed to apply for a normal renewal. You may however, if you so desire and if it can be justified by exceptional circumstances, apply for a one-off renewal period of one year. Therefore, we ask you to provide detailed arguments in order to facilitate the decision of the authorities on the exceptional nature or not of these circumstances.</p>
<p>In order to gain access to the procedure, simply click on the following link: <a href="https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088">https://uclouvain.atlassian.net/servicedesk/customer/portal/11/article/926483088</a></p>

<p>Regards,<br />
The Human Resources<br />
Contact: Marie-Dominique Heureux<br />
If you have been hired on the Saint-Louis site, the contact is Catherine Demain</p>'
WHERE id = 173;"""),

    ]